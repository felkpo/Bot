...
module.exports = {
  ...
  async execute(role) {
    ...
    try {
      ...
      if ('ROLE_DELETE'.includes('ROLE')) {
        eventData.roleId = role.id;
      }
      if ('ROLE_DELETE'.includes('CHANNEL')) {
        eventData.channelId = role.id;
      }

      auditMemoryManager.addEvent(guild.id, eventData);
      logger.info('[AUDIT EVENT DETECTED]', { action: 'ROLE_DELETE', executorId, targetId: role.id });

    } catch (error) {
      logger.error('Erro ao processar evento de auditoria roleDelete', { error: error.message });
    }
  }
};
